<div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-yellow-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-6 text-xs font-medium tracking-wider text-left text-yellow-700 uppercase">
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                            <div class="relative text-left" x-data="{ open: false }">
                                ...
                                <div x-show="open" x-cloak @click.away="open = false"
                                    ...
                                    <button wire:click="confirmRegionDeletion({{ $region->id }})"
            ...
            <div class="w-full max-w-md max-h-full overflow-hidden bg-white rounded-lg shadow-xl">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">
                        {{ $regionId ? 'Edit Region' : 'Create Region' }}
                    </h3>
    ...
    @if ($confirmingRegionDeletion)
        <div class="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black bg-opacity-50">
            <div class="w-full max-w-md bg-white rounded-lg shadow-xl">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Delete Region</h3>
                    <button wire:click="$set('confirmingRegionDeletion', false)" class="text-gray-400 hover:text-gray-500">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button" wire:click="$set('confirmingRegionDeletion', false)"
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            Livewire.on('region-saved', (event) => {
                // You could use a toast notification library here
                alert(event.message);
            });

            Livewire.on('region-deleted', (event) => {
                // You could use a toast notification library here
                alert(event.message);
            });
        });
